...
function SelectLocation({ SSQ, l }: SelectLocationProps) {
    if (l == undefined) {
        return( <select className = 'select'><option value = "" label = "Loading..."></option></select>)
    }
    
    let lStrings = l.map((locationObj) => locationObj.location); 
    lStrings = lStrings.map((locationObj) => locationObj.indexOf(',') == -1 ? locationObj : locationObj.slice(0, locationObj.indexOf(',')))
    lStrings = lStrings.filter((item, index) => lStrings.indexOf(item) === index);
    ...
    return(
        <>
            <select
                ...
                <option value="" label = "Filter by Building"></option>
                {lStrings.map((ll) => 
